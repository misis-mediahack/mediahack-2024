...
def create_dashboard(csv_dash_data: Path, csv_segment_dict: Path, csv_segments: Path):
    # load data
    dash_data = pd.read_csv(csv_dash_data)
    train_segments = pd.read_csv(csv_segments)
    segment_dict = pd.read_excel(csv_segment_dict)
    ...
    advertisers_rank_by_cost = df.groupby('Advertiser')['Estimated cost RUB'].sum().sort_values(
    ...
    def update_graphs(value):
        ...
        filtered_df = train_segments.merge(filtered_avg_cost_by_ad, on='Advertisement ID').merge(filtered_advertisers,
                                                                                                 on='Advertisement ID')
        ...
        fig1 = px.bar(filtered_df.groupby('Segment')['Estimated cost RUB'].mean().reset_index(), x='Segment',
                      y='Estimated cost RUB')
        ...
        fig4 = px.bar(popular_advertisers, x='Advertiser', y='Count', title="Most Popular Advertisers")
        fig5 = px.bar(
            filtered_df.groupby('Advertiser')['Estimated cost RUB'].sum().sort_values(ascending=False).reset_index(),
            x='Advertiser', y='Estimated cost RUB', title="Advertisers Ranked By Cost")
...
@click.command()
@click.option('--dashboard-data', type=Path, required=True)
@click.option('--segment-dict', type=Path, required=True)
@click.option('--segment-predictions', type=Path, required=True)
@click.option('--debug', type=bool, default=False)
@click.option('--port', type=int, default=6008)
def main(dashboard_data: Path, segment_dict: Path, segment_predictions: Path, debug: bool, port: int):
    app = create_dashboard(dashboard_data, segment_dict, segment_predictions)
